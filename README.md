...
프론트 코드를 바로 작성하기보다, 먼저 Swagger를 기준으로 API 요청/응답 구조를 확인했습니다.  
각 페이지에서 필요한 데이터와 사용자 액션을 정리한 뒤, 백엔드 응답 구조에 맞춰 service layer와 타입을 나누었습니다.

중고거래는 사용자가 상품 정보를 믿고 판단해야 하는 서비스라고 생각했습니다.  
그래서 경매가 시작된 뒤 어떤 정보를 수정할 수 있는지, 언제 취소할 수 있는지, 입찰 이후 어떤 값은 서버 기준으로 다시 맞춰야 하는지를 백엔드 개발자와 먼저 정리했습니다.

특히 찜, 입찰, 상품 등록/수정처럼 클릭이 자주 발생하는 기능은 pending 상태와 서버 상태 동기화를 중요하게 다뤘습니다.  
중복 요청을 막고, 화면에 오래된 찜 수나 현재 입찰가가 남지 않도록 TanStack Query의 invalidate 흐름을 함께 적용했습니다.

상품 목록, 상품 상세, 경매장, 경매 상세 화면은 mock 데이터로 단독 확인할 수 있고, 로그인, 상품 등록, 상품 수정, 입찰, 찜, QnA, 마이페이지 기능은 실제 백엔드 API 연동 구조를 기준으로 구현했습니다.
...
| 핵심 구현 포인트 | 일반 거래/경매 거래 분기, 경매 상태 처리, 입찰 가능 여부 검증, 거래 중 수정/취소 정책, 이미지 수정 정책, pending 처리, TanStack Query 기반 서버 상태 동기화 |
...
키워드, 카테고리, 상품 상태를 조합해 탐색할 수 있고,  
상품 카드에는 거래 방식, 찜 수, 가격 정보, 경매 상태를 함께 보여줍니다.
...
진행 중 / 시작 전 / 종료 / 유찰 / 취소 상태에 따라 카드의 상태 표시와 안내 문구가 달라집니다.
...
찜 상태와 찜 수는 TanStack Query를 통해 서버 상태로 관리합니다.
...
입찰 요청은 실제 API 연동을 기준으로 구현했고, 성공 이후 경매 상세, 상품 상세, 내 입찰 목록 등 관련 데이터를 다시 갱신하도록 구성했습니다.
...
그래서 판매자가 언제 상품을 수정하거나 취소할 수 있는지 상태별 기준을 정리했고, Swagger로 API 요청/응답 구조를 고정한 뒤 백엔드 개발자와 응답 필드와 정책을 맞추며 진행했습니다.
...
이 프로젝트에서는 상품 타입 분기, 경매 상태 처리, 입찰 정책 분리, 거래 중 수정/취소 정책, 이미지 수정 정책, pending 처리, TanStack Query 기반 서버 상태 동기화, API service 계층 분리를 핵심 문제로 잡았습니다.
...
프론트 구현 전에 Swagger를 통해 API 요청/응답 구조를 먼저 맞췄고, 화면에서 필요한 데이터가 어떤 페이지에 필요한지 정리한 뒤 service 함수를 분리했습니다.
...
- 내 QnA 목록
- 찜 여부 / 찜 수
- 입찰 성공 이후 경매 / 상품 데이터 갱신
- 경매 만료 이후 관련 데이터 refetch

mutation 이후에는 `lib/query-keys.ts`에 정리한 query key를 기준으로 관련 데이터를 invalidate합니다.

찜은 사용자가 버튼을 눌렀을 때 즉시 반응하는 것이 중요하다고 판단해 optimistic update를 적용했습니다.  
반면 입찰은 현재가, 최고 입찰자, 즉시 구매가 도달 여부, 경매 종료 여부처럼 서버 검증이 중요한 값이 많기 때문에 optimistic update를 적용하지 않았습니다.

### Zustand

Zustand는 서버 상태가 아니라 클라이언트 UI 상태만 담당합니다.
...
상품 목록, 경매 목록, 찜 목록, 입찰 목록, 상품 상세, 경매 상세처럼 서버와 동기화되어야 하는 데이터는 Zustand에 넣지 않고 TanStack Query 또는 서버 컴포넌트 조회가 담당하도록 분리했습니다.
...
이 정책은 `product-image-policy` 테스트와 상품 수정 interaction 테스트로 확인할 수 있도록 구성했습니다.
...
- 서버에서 검색 조건을 받아 바로 조회할 수 있는 목록 화면은 server component 흐름을 유지합니다.
- 로그인 사용자 기준 데이터나 입력 상태가 많은 화면은 CSR로 처리합니다.
- 홈 화면처럼 정적 소개 중심의 화면은 SSG/ISR 후보로 볼 수 있습니다.
- 실시간 인기 상품이나 진행 중 경매처럼 자주 변하는 데이터가 홈에 들어가면 dynamic rendering 또는 ISR 재검토가 필요합니다.
...
중점적으로 확인할 지표는 Core Web Vitals 기준인 LCP, INP, CLS입니다.

- LCP: 상품 이미지나 상세 대표 콘텐츠가 빠르게 표시되는지 확인
- INP: 찜, 입찰, 탭 전환 같은 사용자 입력 반응 확인
- CLS: 상품 카드 이미지, 상세 대표 이미지, countdown 영역의 레이아웃 흔들림 확인

또한 상품 카드 이미지와 상세 대표 이미지 영역은 aspect-ratio를 고정해 이미지 로딩 전후 레이아웃이 흔들리지 않도록 정리했습니다.

현재는 측정값을 확정해서 적기보다, 개발 환경 console과 Lighthouse로 확인할 수 있는 기반을 만들어둔 상태입니다.  
...
또한 찜, 입찰, 상품 이미지 수정처럼 사용자 상호작용이 중요한 기능은 Testing Library 기반의 UI 행동 테스트로 확장했습니다.
...
- 이미지 수정 정책
  - 일반 상품 이미지 수정 가능
  - `READY` 경매 상품 이미지 수정 가능
  - `RUNNING` 경매 상품 이미지 수정 제한
  - 이미지 0장 submit 방지
...
- 입찰 폼 상호작용
  - invalid 금액 submit 방지
  - pending 중 input / button disabled
  - 정상 금액 submit 시 mutation 호출
...
### 2. Swagger 기준으로 API 요청/응답을 먼저 고정한 이유
...
### 3. 반복 클릭과 서버 상태 동기화로 인한 UI 불일치 문제

| 항목 | 내용 |
| --- | --- |
| Problem | 입찰, 찜, 상품 등록/수정처럼 사용자가 빠르게 반복 클릭할 수 있는 기능에서 중복 요청이나 화면 상태 불일치가 발생할 수 있었습니다. |
| Cause | 요청이 끝나기 전에 같은 액션이 다시 실행되면 서버 상태와 화면 상태가 서로 달라질 수 있습니다. 특히 입찰가, 찜 수, 상품 상태처럼 서버 응답이 기준이 되어야 하는 값은 화면에 오래된 값으로 남을 수 있었습니다. |
| Fix | 요청 중에는 pending 상태를 두어 버튼을 비활성화하고, TanStack Query의 invalidate 흐름으로 서버 응답 이후 최신 데이터를 다시 반영했습니다. 찜은 optimistic update를 적용하고, 입찰은 서버 검증이 중요해 optimistic update를 적용하지 않았습니다. |
| Result | 반복 클릭으로 인한 중복 요청 가능성을 줄이고, 입찰가, 찜 수, 상품 상태처럼 서버와 맞아야 하는 값은 응답 이후 다시 맞추는 흐름으로 정리할 수 있었습니다. |

### 4. 거래 중 상품 수정 범위를 제한한 이유

| 항목 | 내용 |
| --- | --- |
| Problem | 경매가 진행 중인 상품의 가격, 이미지, 시작 시간이 바뀌면 입찰자가 판단한 조건이 달라질 수 있었습니다. |
| Cause | 경매 상품은 입찰이 발생한 이후 판매자가 임의로 정보를 바꾸면 거래 신뢰가 깨질 수 있습니다. |
| Fix | 경매 상태별로 수정 가능 범위를 분리했습니다. `READY` 상태에서는 대부분 수정 가능하지만, `RUNNING` 상태에서는 title, description, auctionEndTime 정도로 제한하고, 이미지는 수정할 수 없도록 했습니다. |
| Result | 판매자는 필요한 최소 정보만 수정할 수 있고, 입찰자는 입찰 당시의 핵심 거래 조건이 유지된다는 신뢰를 가질 수 있도록 정책을 정리했습니다. |

### 5. optimistic update와 서버 검증 기준을 분리한 이유
...
│  ├─ product-edit-image-interaction.test.tsx
│  ├─ product-image-policy.test.ts
