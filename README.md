...
프론트엔드 구현 전에 Swagger를 기준으로 API 요청/응답 구조를 먼저 확인하고, 각 페이지에서 필요한 데이터와 사용자 액션을 정리한 뒤 구현했습니다.  
특히 중고거래 서비스는 거래 신뢰가 중요하다고 판단해, 입찰이 발생한 경매 상품의 수정/취소 정책과 pending 상태 처리, 서버 상태 동기화 흐름을 백엔드 개발자와 함께 맞추며 진행했습니다.
...
| 핵심 구현 포인트 | 일반 거래/경매 거래 분기, 경매 상태 처리, 입찰 가능 여부 검증, 거래 중 수정/취소 정책, pending 처리, service layer 기반 API 분리, TanStack Query 기반 서버 상태 동기화 |
...
찜과 QnA는 API 요청 구조를 기준으로 구현되어 있고, 찜 상태와 찜 수는 TanStack Query를 통해 서버 상태로 관리합니다.
...
사용자 액션이 많은 상품 목록과 경매 상세 화면에서 레이아웃 흔들림과 반응성을 확인하기 위해 Web Vitals 측정 컴포넌트를 추가했습니다.

개발 환경에서는 console을 통해 다음 지표를 확인할 수 있습니다.
...
자세한 측정 기준과 개선 내용은 `docs/performance.md`에 정리했습니다.
